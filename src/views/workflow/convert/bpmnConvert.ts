...
function convertLf2DiagramData(bpmnDiagramData, data) {
  ...
  bpmnDiagramData['bpmndi:BPMNShape'] = data.nodes.map((node) => {
    ...
    const shapeConfig = shapeConfigMap.get(node.type);
    if (shapeConfig) {
...
function getNodeConfig(shapeValue, type, processValue) {
  ...
  const shapeConfig = shapeConfigMap.get(type);
  if (shapeConfig) {
...
function convertlf2BpmnData(data: Object, retainedFields?: string[]) {
  ...
  const bpmnProcessData = { ...processAttributes };
  ...
  const definitions = definitionAttributes;
...
function convertBpmnData2lf(bpmnData: Object) {
  if (bpmnData) {
    return convertBpmn2LfData(bpmnData);
  }
}

function convertlf2BpmnXmlData(data: Object, retainedFields?: string[]) {
  const outData = convertlf2BpmnData(data, retainedFields);
  return lfJson2Xml(outData);
}

function convertBpmnXmlData2lf(bpmnData: Object) {
  const json = lfXml2Json(bpmnData);
  return convertBpmnData2lf(json);
}

export { convertlf2BpmnData, convertBpmnData2lf, convertlf2BpmnXmlData, convertBpmnXmlData2lf }